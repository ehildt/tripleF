...
import { decryptSecret } from './decrypt-secret.helper.js';
...
describe('decryptSecret', () => {
  it('decrypts a valid payload', () => {
    ...
    const keys = new Map([
      [crypto.createHash('sha256').update(key).digest('hex').slice(0, 8), key],
    ]);
    expect(decryptSecret(payload, keys)).toBe('hello world');
  });

  it('throws on a malformed payload', () => {
    expect(() => decryptSecret('garbage', new Map())).toThrow(
  ...
  it('throws on an unknown key fingerprint', () => {
    ...
    expect(() => decryptSecret(payload, new Map())).toThrow(
      'No key for fingerprint',
    );
  });
});
