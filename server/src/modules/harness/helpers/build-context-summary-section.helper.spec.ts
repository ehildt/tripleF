...
describe('buildContextSummarySection', () => {
  it('frames the summary as earlier-turn reference data', () => {
    const section = buildContextSummarySection(
      'Subject: the game Neverness to Everness (NTE).',
    );
  ...
  it('forbids mixing the summary into an unrelated current subject', () => {
    ...
    expect(section).toContain(
      'Use this summary ONLY when the latest message explicitly refers to earlier content',
    );
