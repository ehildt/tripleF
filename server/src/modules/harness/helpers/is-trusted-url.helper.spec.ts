...
describe('isTrustedUrl', () => {
  ...
  it('rejects YouTube JS asset URLs', () => {
    ...
    expect(isTrustedUrl('https://www.youtube.com/static/js/foo.js')).toBe(
      false,
    );
  ...
  it('rejects Google thumbnail proxies', () => {
    ...
    expect(isTrustedUrl('https://t0.gstatic.com/images?q=tbn:Example')).toBe(
      false,
    );
  });
  ...
  it('allows private IP / localhost URLs when explicitly allowed', () => {
    expect(
      isTrustedUrl('https://localhost/article', { allowPrivate: true }),
    ).toBe(true);
  });
