 ...
 * simultaneous tool calls by category so parallel searches (e.g. serperWebSearch
 * and brightDataWebSearch) render as one activity instead of flickering duplicates.
 *
...
export function resolveToolCategory(toolName: string): ToolCategory {
  ...
  if (toolName.startsWith('eodhd')) return 'market';
