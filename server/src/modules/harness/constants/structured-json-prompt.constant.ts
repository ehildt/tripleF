...
export function buildStructuredJsonPrompt(): string {
  return [
    ...
    '{',
    '  "template": "article|news|describe|compare|ocr|summary|evaluation|product|imagelist|videolist|text",',
    ...
    '- language: detect from the latest user message. Never default to English. If genuinely undetectable, omit the field. ALL human-readable text must be in that language.',
    '',
    'FINAL REMINDER:',
...
export const languageCorrectionPrompt = [
  ...
  '',
  '- language: detect from the latest user message. Pick the most likely ISO-639 alpha-2 code; never default to English.',
