...
- galleryItems: an array of image objects when multiple relevant images are available. Each entry must include imageUrl, imageAlt, title, and caption. imageAlt and title MUST be non-empty. When online research returns image URLs, populate galleryItems with the remaining images (excluding any hero), up to imageTargetCount — aim for at least 3 when enough URLs are available; with fewer available images, include all of them. Only use imageSearch URLs (720p minimum); never use low-resolution news thumbnails.
