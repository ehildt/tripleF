...
| ------------------- | ------------------------------------------------------------------------------------- |
| Broker              | KeyDB (`infra.compose.yml`, configured by `apps/server/keydb.conf`)                     |
| Module              | `@triplef/bullmq` + `@triplef/bullmq-logger`                                          |
...
There are **two queues**, split by concern: the `HARNESS_QUEUE` is the user-facing hot path, consumed by the server itself, and the `VECTORIZE_QUEUE` carries background memory work (fact extraction + embedding, cognition jobs, consolidation/reflection/conviction/cluster sweeps, encyclopedia jobs), produced by the server and consumed by the **memory app's** `VectorizeProcessor` (see **1.7**). Queue-depth scaling happens by adding workers behind each queue, not by multiplying queues.
...
Defaults come from `BULLMQ_*` env knobs (validated Joi config): attempts, backoff type/delay, LIFO/priority options, stack-trace limits, worker concurrency, TLS. Effective values are introspectable via `GET /api/v1/bullmq/retry-config`; live state via `GET /api/v1/bullmq/live` (the Settings console polls these).

## Failure handling — the persisted DLQ

BullMQ's own failed set is volatile. 3F therefore mirrors terminally failed jobs into **`DeadLetterJob`** (Prisma/PostgreSQL) with the full envelope and error context:

| Operation          | Where                                             |
| ------------------ | ------------------------------------------------- |
| Inspect records    | `GET /api/v1/dlq/:id` (dashboard DLQ area)        |
| Edit before replay | `PATCH /api/v1/dlq/:id` / `…/upsert`              |
| Re-queue           | `POST /api/v1/dlq/reinstate`                      |
| Purge              | `DELETE /api/v1/dlq/:id`                          |

A `LifecycleService` (`modules/dead-letter/services/lifecycle.service.ts`) coordinates DLQ bookkeeping, including retention pruning driven by the `DLQ_*` envs (see **1.8**).
