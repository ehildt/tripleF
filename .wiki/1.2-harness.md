...
| `memoryWrite` | Post-response fact write — enqueue-only: when the classifier picked `memoryRemember`, the step summarizes the turn's gathered results and enqueues a `memory-write` job; the LLM tool loop (prior-memory search → remember decisions) runs in the vectorize worker (see **1.5** memory). The turn's `memoryRecall` hits are passed separately as `probedMemory` so the write job treats them as ALREADY KNOWN — extend/update, never re-store. |
