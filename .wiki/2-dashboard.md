...
| **SysCtl**    | `sysctl/`                | System control: provider configuration, preprocessing settings, search-engines (per-provider sections for Serper, Bright Data, EODHD + YouTube tools), layouts, interface visibility, chat navigation (scroll mode, media priority), system health, tab visibility, widgets, per-session model/config management, memory (system tab: the memory partition id + the memory cognition space id — empty cognition = lives in the partition — plus the cognition limit system variable and a sources-style cognition read-out panel (structured profile + probed insights) with refresh and armed two-click wipe). |
