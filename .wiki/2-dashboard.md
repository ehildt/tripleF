...
- **Entry:** `apps/dashboard/src/main.ts` → `App.vue` (Pinia + VueQuery + router + i18n plugins). Routing is handled by **vue-router** (`src/router/router.ts`): `/chat`, `/dlq`, `/debug`, `/sysctl`, with `/` redirecting to `/chat`.
