...
The memory app (`apps/memory`) serves its own `/api/v1` surface (Qdrant, lexicon, memory-overrides) and the dashboard talks to it **directly** — the server no longer proxies those routes. In dev the Vite proxy maps `/memory-api` → the memory app's `/api`; in prod nginx does the same.
...
| `GET`    | `/qdrant/memory/cognition` | The AI cognition snapshot of one space: structured profile (Postgres) + derived insights (Qdrant) |
| `POST`   | `/qdrant/memory/cognition/insights` | Store one derived insight into the AI's cognition space (sync); `{accepted, id}` envelope |
