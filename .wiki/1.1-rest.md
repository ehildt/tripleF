...
Memory system variables (sysctl → system): global settings layered over env defaults, persisted in the shared `provider_overrides` row `memory`, restored on boot. Currently `cognitionLimit` — the cognition profile character cap (env baseline `MEMORY_COGNITION_LIMIT`, default 5000, clamped 500–32000).
