...
describe('buildCognitionNodes', () => {
  ...
  it('turns each non-empty profile key into its own topic', () => {
  ...
  it('expands object fields into sub-key leafs', () => {
    ...
    expect(nodes[1].topicKey).toBe('communication');
  ...
  it('topics insights by the top-level path segment', () => {
    const nodes = buildCognitionNodes(null, [
    ...
    expect(nodes.map((n) => n.topicKey)).toEqual(['likes', 'likes', 'work']);
  });

  it('groups pathless insights under the insights topic', () => {
    const nodes = buildCognitionNodes(null, [makeInsight('a', 'a')]);

    expect(nodes[0].topicKey).toBe('insights');
  });
});
