...
describe('prepareConstellation', () => {
  it('keeps topics fully expanded and appends the ZERO root', () => {
    const nodes = [
      makeNode('a', 'work'),
      makeNode('b', 'work'),
      makeNode('c', 'hobby'),
    ...
    expect(result.nodeList.some((n) => n.isTopic)).toBe(false);
  ...
  it('collapses a topic whose key is in collapsedKeys', () => {
    ...
    expect(result.nodeList.filter((n) => n.isTopic)).toHaveLength(1);
    expect(result.nodeList.find((n) => n.isTopic)?.id).toBe('topic:work');
    expect(result.nodeList.find((n) => n.isTopic)?.label).toBe('work');
    expect(result.nodeList.find((n) => n.isTopic)?.memberCount).toBe(3);
  });

  it('expands a topic when its key is not in collapsedKeys', () => {
    const nodes = Array.from({ length: 3 }, (_, i) =>
      makeNode(`n${i}`, 'work'),
    );
    const result = prepare(nodes, []);
  ...
  it('appends one always-visible category hub per category (even a lone one)', () => {
    ...
    const clusterNodes = result.nodeList.filter((n) => n.isCluster);
    expect(clusterNodes.map((n) => n.id).sort()).toEqual([
      'cluster:games',
      'cluster:pets',
    ]);
    expect(result.positions.get('cluster:games')).toBeDefined();
    expect(result.hubIds.has('cluster:games')).toBe(true);
  });
  ...
  it('connects related same-category sub-categories with a sibling edge', () => {
    ...
    const cluster = result.linkIndices.filter((l) => l.kind === 'cluster');
    expect(cluster).toHaveLength(2); // nte hub + waves hub → games hub
  ...
  it('emits no intra edges for a collapsed topic', () => {
    const nodes = Array.from({ length: 3 }, (_, i) =>
      makeNode(`n${i}`, 'work'),
    );
    const result = prepare(nodes, [], new Set(['work']));
  ...
  it('aggregates cross-topic links into one inter edge (max score)', () => {
    const nodes = [
      makeNode('a', 'x'),
      makeNode('b', 'x'),
      makeNode('c', 'y'),
  ...
  it('uses the category dot as the main dot for a collapsed topic', () => {
    const nodes = [
    ...
    expect(ids).toContain('topic:work');
  });
  ...
  it('centers each topic fog on its main dot', () => {
    const nodes = [makeNode('a', 'x'), makeNode('b', 'x'), makeNode('c', 'x')];
    const links = [makeLink('a', 'b', 0.9)];
    const result = prepare(nodes, links);

    const fog = result.topicFog.find((f) => f.key === 'x');
  ...
  it('places a collapsed category dot at its members relaxed centroid', () => {
    ...
    const category = result.nodeList.find((n) => n.isTopic);
    const centroid = result.positions.get(category!.id);
    expect(centroid).toBeDefined();
    const fog = result.topicFog.find((f) => f.key === 'work');
    expect(fog?.center).toEqual(centroid);
  });

  it('shows weak links by default and hides them when showSuggested is off', () => {
    // Three topics so each edge aggregates its own topic pair.
    const nodes = [makeNode('a', 'x'), makeNode('c', 'y'), makeNode('e', 'z')];
    const weakLinks: ConstellationLink[] = [
      // A suggested (topical) edge — weak regardless of score.
      {
        source: 'a',
        target: 'c',
        type: 'semantic',
      ...
      // A semantic edge barely clearing the bar — weak by score.
      { source: 'a', target: 'e', type: 'semantic', score: 0.72 },
      // A strong enforced edge — stays a solid link.
      { source: 'c', target: 'e', type: 'semantic', score: 0.95 },
    ];

    const shown = prepare(nodes, weakLinks, new Set(), 0.7);
    expect(shown.linkIndices.filter((l) => l.weak)).toHaveLength(2);
    expect(
      shown.linkIndices.filter((l) => l.kind === 'inter' && !l.weak),
    ).toHaveLength(1);

    const hidden = prepareConstellation(
      nodes,
      buildRelaxedLayout(nodes, weakLinks, 0.7),
      weakLinks,
      new Set(),
      0.7,
      [],
    ...
    expect(hidden.linkIndices.filter((l) => l.weak)).toHaveLength(0);
    expect(hidden.linkIndices.filter((l) => l.kind === 'inter')).toHaveLength(
      1,
    );
  });
});
