...
vi.mock('../../composables/use-sysctl-config', () => ({
  useSysctlConfig: () => ({
    config: ref({ ollama: { apiKey: 'abcd****wxyz', host: 'https://ollama' } }),
    isLoading: ref(false),
    hasError: ref(false),
    resetProvider: sysctlState.resetProvider,
    patchConfig: sysctlState.patchConfig,
    updateApiKey: sysctlState.updateApiKey,
  }),
}));
...
describe('useOllamaConnection', () => {
  ...
  it('saves the API key and refetches the models on success', async () => {
    sysctlState.updateApiKey.mockResolvedValue(true);
    ...
    expect(sysctlState.updateApiKey).toHaveBeenCalledWith(
      'ollama',
      'new-secret-key',
    );
  ...
  it('does not refetch the models when the key save fails', async () => {
    sysctlState.updateApiKey.mockResolvedValue(false);
    const modelsStore = useModelsStore();
    const fetchModels = vi
      .spyOn(modelsStore, 'fetchModels')
      .mockResolvedValue(undefined);
  ...
  it('saves the host and refetches the models', async () => {
    ...
    expect(sysctlState.patchConfig).toHaveBeenCalledWith(
      'ollama',
      'host',
