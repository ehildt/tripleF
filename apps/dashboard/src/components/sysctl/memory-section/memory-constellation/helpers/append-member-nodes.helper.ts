 ...
 * (hidden) — it reads as a stray dot, so it folds back into the cluster.
 */
...
/** Append an expanded/small cluster's members at their relaxed positions. */
export function appendMemberNodes(
  ...
  for (const memberId of cluster.memberIds) {
    ...
    // (folded back into the cluster) rather than shown as a stray dot.
