...
describe('ExchangeHeader meta bar', () => {
  ...
  it('renders the merge tags below the meta row for a merged user exchange', () => {
    const wrapper = mountHeader(
      makeExchange({
        role: 'user',
        status: 'done',
    ...
    // For a user exchange the meta row IS the header (the assistant header
    // block is v-if="!isUser"), and the merge marker sits right below it.
    expect(order).toEqual(['meta-row', 'merge-tags']);
  });
