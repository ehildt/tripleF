...
import { mapTaggedModelOrigin } from './map-tagged-model-origin.helper.js';

describe('mapTaggedModelOrigin', () => {
  it('stamps a local origin when the host is not cloud', () => {
    expect(
      mapTaggedModelOrigin({ name: 'llama3', details: {} }, false),
    ).toEqual({ name: 'llama3', details: {}, origin: 'local' });
  });
