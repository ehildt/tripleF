...
function publishedAfterFor(recency?: SearchRecency): string | undefined {
  ...
  return new Date(
    Date.now() - RECENCY_DAYS[recency] * 86_400_000,
  ).toISOString();
}
...
export function createYoutubeVideoSearch(deps: ToolDependencies) {
  return tool({
    description:
    ...
    execute: async ({
      ...
      deps.logger.log(
        `YouTube video search returned ${results.length} results for "${query}"`,
      );
      return { results };
    },
  });
