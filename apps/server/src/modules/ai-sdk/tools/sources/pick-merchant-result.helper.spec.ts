...
describe('pickMerchantResult', () => {
  it('returns undefined for an empty store token', () => {
    expect(pickMerchantResult([{ url: 'https://example.com' }], '')).toBe(
      undefined,
    );
  });
  ...
  it('picks the first result whose host carries the store token', () => {
    const results = [
      { url: 'https://www.scancomputers.com/product' },
      { url: 'https://other.com' },
    ];
    expect(pickMerchantResult(results, 'scancomputers')).toBe(
      'https://www.scancomputers.com/product',
    );
  });
