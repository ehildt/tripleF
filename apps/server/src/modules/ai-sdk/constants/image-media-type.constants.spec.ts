...
import {
  ...
  MEDIA_TYPE,
} from './image-media-type.constants.js';
  ...
  describe('matchesMediaTypeSignature', () => {
    it('returns true when all non-null bytes match', () => {
      ...
      expect(
        matchesMediaTypeSignature(
          buffer,
          IMAGE_MEDIA_TYPE_SIGNATURES[MEDIA_TYPE.PNG],
        ),
      ).toBe(true);
    });
    ...
    it('ignores null wildcards', () => {
      ...
      expect(
        matchesMediaTypeSignature(
          buffer,
          IMAGE_MEDIA_TYPE_SIGNATURES[MEDIA_TYPE.GIF],
        ),
      ).toBe(true);
    });
    ...
    it('returns false when buffer is shorter than signature', () => {
      ...
      expect(
        matchesMediaTypeSignature(
          buffer,
          IMAGE_MEDIA_TYPE_SIGNATURES[MEDIA_TYPE.JPEG],
        ),
      ).toBe(false);
    });
