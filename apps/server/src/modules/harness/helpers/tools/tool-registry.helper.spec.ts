...
describe('tool-registry.helper', () => {
  describe('categorizeTools', () => {
    it('groups tools by functional category', () => {
      ...
      // Web search tools have no dedicated category — they land in
      // `specialized` alongside the other uncategorized concrete tools.
      expect(groups.specialized).toContain('serperWebSearch');
      expect(groups.specialized).toContain('brightDataWebSearch');
  ...
  describe('getEnabledToolNames', () => {
    it('includes only tools whose provider is enabled and configured', () => {
      ...
      expect(enabled).toContain('brightDataWebSearch');
      expect(enabled).toContain('serperImageSearch');
      expect(enabled).toContain('webFetch');
      expect(enabled).toContain('eodhdSearch');
      expect(enabled).toContain('eodhdQuote');
      expect(enabled).toContain('eodhdHistory');
      expect(enabled).toContain('eodhdTechnical');
      expect(enabled).toContain('eodhdNews');
      expect(enabled).toContain('eodhdFundamentals');
    });

    it('excludes EODHD tools when EODHD is disabled', () => {
      const enabled = getEnabledToolNames({
        ...fullConfig,
        eodhd: { ...fullConfig.eodhd, enabled: false },
      });

      expect(enabled).not.toContain('eodhdSearch');
      expect(enabled).not.toContain('eodhdQuote');
      expect(enabled).not.toContain('eodhdHistory');
    });
  ...
  describe('expandToolAliases', () => {
    it('keeps concrete enabled tool names unchanged', () => {
      ...
      const expanded = expandToolAliases(
        ['serperWebSearch', 'serperImageSearch'],
        enabled,
      );

      expect(expanded).toEqual(['serperWebSearch', 'serperImageSearch']);
    });
