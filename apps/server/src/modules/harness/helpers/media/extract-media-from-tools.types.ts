...
export interface ExtractedImageItem {
   ...
   * skips the configured dimension floor (the effective pproc resize
   * dimensions). Used for engines (e.g. Bright Data) whose results we trust
   * the Google-side size filter for — Google images are usually og-image
   * size (~1200×630), potentially below the configured floor.
   */
