 ...
 * contributes a distinct candidate set for the visual verification step.
 * Queries must progress from GRANULAR (broad subject) to SPECIFIC (narrow
 * clues): a narrow-only set backfires when the specific guess is wrong,
 * because the broad query is what surfaces the subject's name.
 */
...
- Call EVERY selected *ImageSearch tool at least once, one call per enabled provider (e.g. serperImageSearch and brightDataImageSearch) — providers return different result sets and later steps verify every candidate visually against the uploaded image(s), so extra candidates are filtered, not harmful.
