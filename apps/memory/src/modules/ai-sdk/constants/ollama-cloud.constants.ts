...
/**
 * How often the model-origin catalog is re-fetched in the background. The
 * memory app has no models endpoint (unlike the server, where the SysCtl
 * models tab warms the catalog lazily), so without a periodic refresh a
 * newly added cloud model would keep routing to the local Ollama host
 * until the next restart.
 */
export const MODELS_REFRESH_INTERVAL_MS = 60_000;
