...
export const MEMORY_CONSOLIDATE_INSTRUCTIONS = `MEMORY CONSOLIDATION — one purpose: decide whether a NEW FACT stored this sweep window is already covered by EXISTING CANDIDATES from the same memory space, and resolve redundancy without losing information or provenance.
...
OUTPUT FORMAT — output ONLY valid JSON: {"verdict": "keep" | "redundant" | "merge", "mergedText"?: string}
...
function renderLine(line: ConsolidateProvenanceLine): string {
  ...
  const when = line.createdAt
    ? ` on ${new Date(line.createdAt).toISOString().slice(0, 10)}`
    : '';
