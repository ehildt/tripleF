...
import type { LexiconSweepJobData } from '../../../models/memory.model.js';
...
/**
 * Lexicon supersede sweep job handler (vectorize queue): heals orphaned
 ...
 */
@Injectable()
export class LexiconSweepService {
  private readonly logger = new Logger(LexiconSweepService.name);
  ...
  async execute(data: LexiconSweepJobData): Promise<void> {
    const limit = Math.min(data.limit ?? 100, MAX_PENDING_PER_RUN);
    const pending = await this.ledger.listPending(limit);
    if (pending.length === 0) {
      this.logger.debug('lexicon-consolidate: nothing pending');
      return;
    }
    ...
    for (const row of pending) {
      ...
      if (currentHash && currentHash !== row.contentHash) {
        if (data.dryRun) {
          this.logger.log(
            `lexicon-consolidate [dryRun]: would heal ${row.url} (${row.contentHash} → ${currentHash})`,
          );
          continue;
        }
    ...
    this.logger.log(
      `lexicon-consolidate: processed ${pending.length} — healed ${healed} superseded documents${data.dryRun ? ' (dryRun)' : ''}`,
    );
  }
}
