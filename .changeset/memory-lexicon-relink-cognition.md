...
- **Encyclopedia persistence**: new `memory-encyclopedia` collection stores verbatim chunks of fetched sources (deterministic `url|contentHash|chunkIndex` ids, global scope with `partitionScope` provenance, `content`/`result` source types), plus a encyclopedia sweep job and read-through cache accounting.
