...
- `@triplef/agent/schemas` — the Zod schemas and `z.infer` types for intent classification, response templates, and memory extraction/consolidation/profile, plus the URL-trust and Zod-shape helpers (`formatZodShape`, `deriveSchemaKeys`), and the `LexiconSelectInput`/`LexiconSelectResult` contract for the ephemeral retrieval-selection endpoint.
