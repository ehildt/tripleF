...
describe('retryWithBackoff', () => {
  ...
  it('retries until the task succeeds', async () => {
    ...
    await expect(retryWithBackoff(task, { attempts: 3, initialDelayMs: 1 })).resolves.toBe('ok');
    expect(task).toHaveBeenCalledTimes(2);
  });

  it('rethrows the last error when attempts are exhausted', async () => {
    const task = vi.fn().mockRejectedValue(new Error('boom'));
    await expect(retryWithBackoff(task, { attempts: 2, initialDelayMs: 1 })).rejects.toThrow('boom');
    expect(task).toHaveBeenCalledTimes(2);
  });
