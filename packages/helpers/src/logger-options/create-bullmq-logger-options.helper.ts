...
/**
 * Build the pino `LoggerOptions` for the BullMQ job logger from env. Mirrors
 * the core logger: pretty-prints outside production (override with
 * `BULLMQ_LOG_PRETTY`) and redacts secret-shaped fields. The custom
 * `timestamp` keeps an ISO string under the `timestamp` key (pino's default
 * `time` is epoch millis).
 */
export function createBullMQLoggerOptions(env = process.env): LoggerOptions {
  ...
  return {
    ...
    transport: prettyEnabled
      ? {
          ...
          options: {
            translateTime: env.BULLMQ_LOG_TRANSPORT_TRANSLATE_TIME || 'yyyy-mm-dd HH:MM:ss.l',
