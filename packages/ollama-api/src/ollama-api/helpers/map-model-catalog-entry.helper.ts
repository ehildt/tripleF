import type { ModelCatalogEntry, ShowResult, TaggedModel } from '../ollama-api.model.ts';
...
export function mapModelCatalogEntry(
  ...
  return {
    model: entry.name,
    origin: entry.origin,
    parameter_size: entry.details?.parameter_size || show?.details?.parameter_size,
    quantization_level: entry.details?.quantization_level || show?.details?.quantization_level,
