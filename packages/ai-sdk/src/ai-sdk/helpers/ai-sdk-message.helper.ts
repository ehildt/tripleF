import type { AiSdkMessages, InputMessage } from '../types/ai-sdk-messages.types.ts';

import { toAiSdkMessage } from './to-ai-sdk-message.helper.ts';
...
function extractSystemPrompt(messages: InputMessage[]): string | undefined {
  const systemContents = messages.filter(isSystemMessage).map((message) => message.content);
...
export function toAiSdkMessages(messages: InputMessage[]): AiSdkMessages {
  ...
  const convertedMessages = messages.filter((message) => !isSystemMessage(message)).map(toAiSdkMessage);
