...
import { UnsupportedImageFormatError } from '../constants/image-format-errors.ts';
import { MEDIA_TYPE } from '../constants/image-media-type.constants.ts';

import { detectImageMimeType } from './detect-image-mime-type.helper.ts';

describe('detectImageMimeType', () => {
  it.each([
    [MEDIA_TYPE.PNG, [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]],
    [MEDIA_TYPE.WEBP, [0x52, 0x49, 0x46, 0x46, 0x00, 0x00, 0x00, 0x00, 0x57, 0x45, 0x42, 0x50]],
  ...
  it('throws when format is not supported', () => {
    expect(() => detectImageMimeType(Buffer.from([0x00, 0x00, 0x00]))).toThrow(UnsupportedImageFormatError);
  });
});
