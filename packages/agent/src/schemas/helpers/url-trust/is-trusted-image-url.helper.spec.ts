...
describe('isTrustedImageUrl', () => {
  ...
  it('allows direct image file URLs from public hosts', () => {
    ...
    expect(isTrustedImageUrl('https://news.example.com/photo.webp?size=large')).toBe(true);
  });
  ...
  it('allows known trusted image hosts even without file extension', () => {
    ...
    expect(isTrustedImageUrl('https://upload.wikimedia.org/wikipedia/commons/thumb/1/2/3/example')).toBe(true);
  });
  ...
  it('rejects Google thumbnail proxies', () => {
    ...
    expect(isTrustedImageUrl('https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcExample')).toBe(false);
    expect(isTrustedImageUrl('https://t0.gstatic.com/images?q=tbn:Example')).toBe(false);
  });
  ...
  it('allows localhost and private IP addresses for direct image files', () => {
    ...
    expect(isTrustedImageUrl('http://localhost:9000/bucket/photo.webp')).toBe(true);
  });

  it('rejects localhost/private URLs without an image extension or trusted host', () => {
    expect(isTrustedImageUrl('https://localhost/article-image?id=123')).toBe(false);
  ...
  it('rejects URLs without image extension from unknown hosts', () => {
    expect(isTrustedImageUrl('https://unknown-site.com/article-image?id=123')).toBe(false);
  });
  ...
  it('detects blocked image hosts independently of extension', () => {
    expect(
      hasBlockedImageHost(
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRaM3gHelAVzTqaCbfFsh03K6esoOW4PvFABV_lmer6iEZFDpVZ1ZptA8U&usqp=CAI&s',
      ),
    ).toBe(true);
    expect(hasBlockedImageHost('https://example.com/photo.jpg')).toBe(false);
    expect(hasBlockedImageHost('/api/v1/storage/req-1/0')).toBe(false);
    expect(hasBlockedImageHost('data:image/png;base64,abc')).toBe(false);
  });
});
