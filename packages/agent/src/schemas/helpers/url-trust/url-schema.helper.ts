 ...
 */
export function safeUrl(message: string | { message: string } = 'must be a safe URL') {
 ...
 */
const LOCAL_STORAGE_URL_PATTERN = /^\/api\/v1\/storage\/[A-Za-z0-9_-]+\/[A-Za-z0-9_-]+\/[A-Za-z0-9]+$/;
 ...
 */
export function safeVideoUrl(message: string | { message: string } = 'must be a safe video URL') {
  const text = typeof message === 'string' ? message : message.message;
 ...
 */
export function safeVideoUrlOrEmpty(message: string | { message: string } = 'must be a safe video URL') {
 ...
 */
export function safeMediaUrl(message: string | { message: string } = 'must be a safe media URL') {
  const text = typeof message === 'string' ? message : message.message;
  return z
    .string()
    .refine((value) => LOCAL_STORAGE_URL_PATTERN.test(value) || isAbsoluteHttpUrl(value), { message: text });
}

/**
 * Zod schema that accepts an empty string OR a safe media URL.
 */
export function safeMediaUrlOrEmpty(message: string | { message: string } = 'must be a safe media URL') {
