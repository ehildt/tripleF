import { RESPONSE_LAYOUTS, type ResponseLayout } from './response-layout.constant.js';
...
export function buildLayoutInstruction(allowed: ResponseLayout[]): string {
  ...
  const lines = usable
    .filter((layout): layout is ResponseLayout => (RESPONSE_LAYOUTS as readonly string[]).includes(layout))
