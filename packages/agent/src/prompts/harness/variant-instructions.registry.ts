...
import { OCR_INSTRUCTIONS, OCR_VERBATIM_INSTRUCTIONS } from '../instructions/ocr.instruction.js';
 ...
 */
export function resolveVariantInstructions(template: string, variantId: string = DEFAULT_VARIANT_ID): string {
  const instructions =
    VARIANT_INSTRUCTIONS[`${template}:${variantId}`] ?? VARIANT_INSTRUCTIONS[`${template}:${DEFAULT_VARIANT_ID}`] ?? '';
