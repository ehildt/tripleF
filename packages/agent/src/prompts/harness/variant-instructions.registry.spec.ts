...
import { DEFAULT_VARIANT_ID, resolveVariantInstructions, TEMPLATE_VARIANTS } from './variant-instructions.registry.js';
...
describe('resolveVariantInstructions', () => {
  it('returns instructions for a known template and variant', () => {
    expect(resolveVariantInstructions('describe', 'detailed')).toContain('exhaustive visual detail');
  });
  ...
  it('falls back to the default variant when an unknown variant is requested', () => {
    ...
    expect(fallback).toBe(resolveVariantInstructions('describe', DEFAULT_VARIANT_ID));
