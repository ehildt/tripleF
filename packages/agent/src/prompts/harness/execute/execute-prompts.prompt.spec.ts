...
import type { IntentResult } from '../../../schemas/index.js';

import { buildImageExecutePrompt } from './image-execute.prompt.js';
import { buildToolExecutePrompt } from './tool-execute.prompt.js';
...
describe('buildToolExecutePrompt', () => {
  it('states the standalone query rules for every search query', () => {
    ...
    expect(prompt).toContain('NEVER copy the latest user message into a query verbatim');
  ...
  it('declares subject-less queries a failure above the endpoint guidance', () => {
    ...
    expect(prompt).toContain('A query without its explicit subject is a failure');
    expect(prompt.indexOf('STANDALONE QUERY RULES')).toBeLessThan(prompt.indexOf('QUERY CRAFTING'));
  });
  ...
  it('lists the mandatory tools from the intent', () => {
    const prompt = buildToolExecutePrompt(buildIntent({ tools: ['serperWebSearch', 'serperNewsSearch'] }));

    expect(prompt).toContain('MANDATORY tools you MUST call: serperWebSearch, serperNewsSearch');
  });
  ...
  it('guides freshness decisions: evergreen queries get no year, time-sensitive ones may', () => {
    ...
    expect(prompt).toContain('do NOT add a year qualifier — it filters out good results');
    expect(prompt).toContain('vary the parallel queries: some fresh/dated, some with no year');
  });
...
describe('buildImageExecutePrompt', () => {
  it('teaches per-endpoint query crafting from visible signals', () => {
    ...
    expect(prompt).toContain('*WebSearch: short factual identification queries');
    expect(prompt).toContain('*ImageSearch: a short standalone visual description of the subject');
  ...
  it('requires every selected tool as parallel calls in one response', () => {
    ...
    expect(prompt).toContain('Emit EVERY selected tool call in ONE response as parallel tool calls');
  });
