 ...
 * space: the EXCLUSIVE store for subjective user data (preferences and
 * interests, stated or derived), holding ONE structured profile document
 * (stable identity and durable traits) plus any new derived INSIGHTS
 * (topic-specific understanding recalled later by embedding probe when the
 * conversation touches their topics). The objective fact partition never
 * carries subjective user data — that boundary is enforced by rule.
 *
...
You hold three kinds of cognition, all SUBJECTIVE (never the objective fact store):
...
- Exclusive home of the subjective: THIS cognition tier is the ONLY store for user profile data — stated preferences and interests ("the user likes…", "the user is interested in…") as well as derived durable characteristics, goals, expertise, communication style. When the user STATES a preference or interest, record it here (a short profile topic, plus an insight when there is depth) — never skip it as "a fact for the memory records": the objective fact partition is barred from carrying preferences, so a preference you do not record is lost. Never one-off moods, task content, or objective facts (contacts, project details, events) — objective records belong to the memory partition, never to this profile.
