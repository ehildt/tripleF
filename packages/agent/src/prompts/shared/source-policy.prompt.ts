/**
 * Minimal source-policy shape the prompt needs — the server's SourcesConfig
 * satisfies it structurally, so the prompt stays decoupled from app config.
 */
 ...
 */
export function buildSourcePolicyPrompt(sources?: SourcePolicyConfig): string {
...
function hasPatternEntries(...lists: string[][]): boolean {
  return lists.some((list) =>
    list.some((entry) => entry.startsWith('*.') || (entry.length > 2 && entry.startsWith('/') && entry.endsWith('/'))),
  );
}
