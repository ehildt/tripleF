...
describe('processHarnessResponseEvent', () => {
  ...
  it('buffers chart data hidden until the respond step streams', () => {
    let state = createHarnessResponseState('req-1');

    // Chart data arrives right after the tool runs — buffered, not revealed.
    state = processHarnessResponseEvent(state, {
      template: 'stockmarketitem',
      chartData: {
        toolName: 'eodhdHistory',
        data: { history: [{ time: '2024-01-01', close: 100 }] },
      },
    });
    expect(state.chartData.eodhdHistory).toEqual({
      history: [{ time: '2024-01-01', close: 100 }],
    });
    expect(state.revealCharts).toBe(false);

    // The first respond delta reveals the buffered charts.
    state = processHarnessResponseEvent(state, {
      template: 'stockmarketitem',
      delta: '{"title":"NVDA"',
    });
    expect(state.revealCharts).toBe(true);
  });
});
